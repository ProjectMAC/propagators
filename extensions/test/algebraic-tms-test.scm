...
(in-test-group
 ...
 (define-test (smoke)
   (interaction
    ...
    (define-cell bill-cons (e:copy-cons nothing bill))
    (define-cell answer)
    (c:== bill-cons answer)
    (define-cell fred (make-tms (supported 4 '(fred))))
    (define-cell fred-cons (e:copy-cons fred nothing))
    (define-cell george (make-tms (supported #t '(george))))
    (conditional-wire george fred-cons answer)
    (define-cell the-pair? (e:copy-pair? answer))
    (define-cell the-car (e:copy-car answer))
    (define-cell the-cdr (e:copy-cdr answer))
    (run)
    ; (pp (content answer))
    (content the-pair?)
    (produces #t)
