...
(define (general-propagator-apply closure-cell arg-cells)
...
(define (eager-diagram-apply closure arg-cells)
  (if (diagram-style? closure)
...
(define (p:application closure-cell . arg-cells)
  (let ((arg-cells (map ensure-cell arg-cells)))
	...
	(if (directly-applicable? (content closure-cell))
	    (eager-diagram-apply (content closure-cell) arg-cells)
	    (general-propagator-apply closure-cell arg-cells))
	(if (directly-applicable? closure-cell)
	    (eager-diagram-apply closure-cell arg-cells)
	    (general-propagator-apply (ensure-cell closure-cell) arg-cells)))))
...
(define (application closure-cell . arg-cells)
  (let ((arg-cells (map ensure-cell arg-cells)))
    (define (eager-apply closure arg-cells)      
	  ...
	  (eager-diagram-apply closure arg-cells)
	  (if (diagram-style? closure)
    ...
    (if (cell? closure-cell)
	(if (directly-applicable? (content closure-cell))
	    (eager-apply (content closure-cell) arg-cells)
	    (general-propagator-apply closure-cell arg-cells))
	(if (directly-applicable? closure-cell)
	    (eager-apply closure-cell arg-cells)
	    (general-propagator-apply (ensure-cell closure-cell) arg-cells)))))
